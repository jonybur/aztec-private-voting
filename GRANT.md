...
**PIUP Study 2 — explanation and calibration effects (pre-registration in progress):** Study 1 establishes whether label choice determines privacy mental model quality. Study 2 asks whether the label is the load-bearing element, or whether protective framing can substitute for a good label. A pre-registered 2×2×2 factorial (Label × Explanation × Calibration intervention; N=240, 8 cells, n=30 per cell) tests whether absent-choice explanation closes the performance gap between *vote fingerprint* and *confirmation code* on the two privacy-specific items (Q2: does the identifier prove vote choice?; Q3: does showing the receipt to a coercer reveal the vote?). The calibration factor (a comprehension pre-check with immediate feedback before the receipt display) tests whether active correction narrows the gap further. If the primary Label×Explanation interaction is null — if protective framing does not close the privacy-item gap between *vote fingerprint* and *confirmation code* — the label is the load-bearing element: framing cannot substitute for a representationally correct identifier. If the interaction is significant, the gap can be rehabilitated through explanation, and the design problem becomes a copywriting problem, not an identifier-choice problem. Either outcome is informative. The pre-registration draft is complete (`docs/piup-study2-preregistration-draft-2026-06-29.md`); OSF filing is gated on Study 1 pilot data (N=40) to calibrate baseline Q-accuracy estimates for power analysis, and on Study 1's H4 outcome to determine the final Study 2 sample size (N=240 if H4 supported; N=160 if not). Design specification, survey instrument, analysis script, and Prolific setup guide are committed to this repo.

**PIUP Study 3 — social verification pilot (pre-registered):** Study 3 tests whether a social proof signal embedded in the PIUP receipt increases post-vote verification return rates — the proportion of voters who return at T+14 to confirm their ballot was counted. Social proof (Cialdini, 1984) has been shown to increase upfront security behaviour (Das et al., CCS 2014: password manager adoption via peer-count display); Study 3 asks whether the same mechanism operates for *deferred* security behaviour in a private voting context. The design is a two-arm between-subjects field experiment (N=80 pilot; n=40/condition; control: standard PIUP receipt; treatment: receipt + live counter of how many voters have already verified). The counter is maintained by a host-side backend (serverless API + KV store): when a participant calls `verify_vote_counted()` via the receipt UI, the host logs a timestamped aggregate verification event — outcome only, no wallet address — and serves the cached count (refreshed every 15 minutes) to the receipt component. Aggregate verification counts are tracked without de-anonymising any individual voter. (Architecture note: `verify_vote_counted()` is a `#[view]` function in the Noir contract — it executes locally as a node simulation and leaves no on-chain record. The count is host-server-maintained, consistent with the pre-registered privacy guarantee.) The pilot is powered to estimate a 90% CI on the verification-rate odds ratio; a confirmatory replication (N≥280) follows if the OR is positive. Pre-registration, analysis script, and debrief script are committed to this repo ([`docs/piup-study3-osf-prereg-2026-07-01.md`](docs/piup-study3-osf-prereg-2026-07-01.md), [`analysis/piup-study3-analysis.R`](analysis/piup-study3-analysis.R)).
...
| Alpha testnet deployment | ⚠️ **Awaiting deployer keys** — v5 testnet RPC confirmed live (block 2203, Jul 2 2026 ~09:13 UTC, ~50 blocks/hr post-second-reset); deploy command ready in `docs/v5-upgrade-runbook.md` |
