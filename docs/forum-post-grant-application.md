...
**Temporal constraint UX (Invariant 2):** A design spike (Jul 2026, [`docs/piup-temporal-disclosure-ux-spike-2026-07-01.md`](docs/piup-temporal-disclosure-ux-spike-2026-07-01.md)) analyzed four approaches for surfacing Invariant 2 (the surrogate must be treated as private until the vote closes) to voters: copy-only (current), deferred share UI lock, two-phase receipt, and countdown. **Option D — countdown timer** (*"Sharing is safe in X days Y hours"*) is recommended for immediate implementation: it makes the temporal constraint salient without adding behavioral friction or requiring enforcement. **Option B — UI lock** is the upgrade path for adversarial-context deployments where voters need social deniability (the ability to truthfully say *"the app won't let me share it yet"*). The deniability hypothesis (Option B users are more resistant to coercion pressure because they have a technically-grounded exit) is flagged as a Study 4 candidate — a vignette experiment with a UI-lock × social-pressure factorial design.
...
**PIUP Study 3 — social verification (pre-registered):** A two-arm between-subjects field pilot (N=80; n=40/condition; control: standard PIUP receipt; treatment: receipt + live counter of how many voters have already verified) testing whether a social proof signal embedded in the receipt increases post-vote verification return rates — the proportion of voters who return at T+14 to confirm their ballot was counted. The counter draws from the contract's public `verify_vote_counted()` call logs (aggregate counts available on-chain without de-anonymising any individual voter); activates after ≥5 verified (pre-registered floor, to avoid negative social proof at low counts). Grounded in Das et al. (CCS 2014), who found aggregated peer-behavior counts increased password-manager adoption — extended here to deferred post-vote verification behavior. The pilot is powered to estimate a 90% CI on the verification-rate odds ratio; a confirmatory replication (N≥280) follows if OR is positive. Pre-registration, analysis script, debrief script, and pre-IRB critique are all committed to this repo ([`docs/piup-study3-osf-prereg-2026-07-01.md`](docs/piup-study3-osf-prereg-2026-07-01.md), [`analysis/piup-study3-analysis.R`](analysis/piup-study3-analysis.R)).

**PIUP Study 4 — Invariant 2 behavioural validation (pre-registered):** Studies 1–3 test receipt comprehension and verification behaviour. Study 4 tests whether the UI-lock that enforces Invariant 2 (download/copy disabled before vote close) produces genuine social deniability under adversarial pressure. A pre-registered 2×2 between-subjects vignette (N=160; UI-lock present/absent × coercion pressure moderate/high; primary DV: self-reported sharing intent, 7-point scale; powered for f=0.25, 86% power) tests the interaction hypothesis that the lock's sharing-intent reduction is larger under high-pressure — where a technical constraint (“I can't share it”) is harder to override than a normative one (“I shouldn't”). This study is entirely Prolific-based and does not require a live contract deployment. Pre-registration, Qualtrics guide, Prolific setup guide, and R analysis script are committed ([`docs/piup-study4-osf-prereg-2026-07-01.md`](docs/piup-study4-osf-prereg-2026-07-01.md), [`analysis/piup-study4-analysis.R`](analysis/piup-study4-analysis.R)).

This four-study programme is the answer to the open question in the [Horizon PRD for Private Voting](https://github.com/AztecProtocol/Horizon/blob/main/PRDs/Private_Voting_Module_for_DAOs.md): *"Default receipts content voters expect."* No other voting tool has asked this question empirically.
...
| §11 Open question: *"Default receipts content voters expect"* | ✅ Answered — PIUP (`docs/proof-of-inclusion-ux-pattern-2026-06-22.md`) + empirical programme: Study 1 pre-registered (N=280, label effects); Study 2 draft pre-registered (N=240, explanation effects); Study 3 pre-registered (N=80 pilot → N≥280 confirmatory, social verification); Study 4 pre-registered (N=160 vignette, UI-lock coercion deniability) |
