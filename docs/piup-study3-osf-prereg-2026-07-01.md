...
_[Amendment tick-4453 — §3.2 architecture correction (pre-data): The original text implies the counter is sourced from on-chain `verify_vote_counted()` call logs. This is architecturally incorrect. `verify_vote_counted()` is declared `#[view]` in the Noir contract; view functions execute locally (node simulation), do not broadcast a transaction, and leave no on-chain record. There is no Aztec mechanism to retrieve a historical count of view-function invocations from chain state. The correct architecture: the counter is maintained by the **host server** (a lightweight serverless API, e.g., Vercel function + KV store). When a participant calls `verify_vote_counted()`, the host backend (a) runs the view simulation on the Aztec node, (b) logs an aggregate verification event — timestamp and outcome only; **no wallet address, no receipt ID** — in the host KV store, and (c) serves the cached verification count (refreshed every 15 minutes) as the `socialProofCount` prop to the receipt component. This is implementation-equivalent to the original design in all observable respects: the count is real, accurate, and updated every 15 minutes. No hypothesis, DV, analysis plan, or sample size change. The privacy guarantee is maintained: only the aggregate count is stored; no individual voter is identifiable. Filed as pre-data architectural clarification before OSF upload. | Jony Bursztyn]_
...
_[Amendment tick-4453 — Privacy note correction (pre-data): "computed from public smart contract logs" is incorrect. `verify_vote_counted()` is a view function; it produces no smart contract logs. The counter is maintained by the host server (see §3.2 amendment). The host stores only the aggregate verification count and timestamps — no wallet addresses and no receipt IDs are retained. The privacy guarantee is identical to the original design: no individual voter is identifiable from the count. The claim "the count is real and accurate" is preserved. Amendment to §3.2 (tick-4453) covers the full architectural correction. Pre-data; no hypothesis or DV change. | Jony Bursztyn]_
