...
**H2.3 contingency:** If Study 1 H4 is NOT supported, drop H2.3 from this pre-registration and reduce N from 240 to 160 (4 cells × 40/cell). Update §4, §7, §12, and §13 before OSF upload. (§9 and §10 have no N references and do not need updating; §7 pilot N: 80→40; §12 budget: ~$1,064→~$798; §13: remove H2.3 row.)
...
**If H4 NOT supported in Study 1:** H2.3 is dropped from this pre-registration. N is reduced from 240 to 160 (4 cells × 40/cell; I factor dropped). Update §4, §7, §12, and §13 accordingly before OSF upload. (§9 and §10 have no N references; §7 pilot N: 80→40; §12 budget: ~$1,064→~$798; §13: remove H2.3 row.) [Fixed Amendment 13 propagation (tick-4270): status header was corrected in Amendment 13 but this body instance still said '§4, §9, and §10' — the same stale list. §9 (Open science commitments) and §10 (Ethical considerations) have no N references and require no update when H2.3 is dropped.]
...
- Browser-fallback participants are **NOT excluded** (see design note §9.3): they are retained in the primary analytic sample and flagged (`browser_fallback = 1`); pre-specified sensitivity checks exclude them from H2.1 and H2.4
...
**H2.1 browser-fallback sensitivity check (pre-specified):** Re-run H2.1 (chi-squared, E1 vs. E2, one-tailed) excluding `browser_fallback = 1` participants. Fallback participants received a static screenshot rather than the interactive prototype; their Q-AC responses may reflect screenshot comprehension rather than interactive prototype comprehension, introducing a nuisance difference across conditions. Report both primary and sensitivity-check proportions, ORs, and verdicts. If browser_fallback = 1 n = 0, note that sensitivity check is not applicable.
...
- If F_interaction not significant: report null with 90% CI on interaction term; report E main effect and L main effect descriptively. [Amendment 16 (pre-data): analysis script null path previously reported the 90% CI on the interaction term (via `emmeans::contrast`, `level = 0.90`) but did not compute or print E and L marginal means. Fixed: `emmeans::emmeans(m2_lm, ~ E)` and `emmeans::emmeans(m2_lm, ~ L)` added to null path. Dead code removed: `int_ci90 <- confint(m2_lm, level = 0.90)` and `int_term` were computed but never used; deleted. No hypothesis, alpha, or verdict logic change. Commit tick-4299.]
...
1. **M4 residual (miscalibration):** Two-sample t-test (I1-L2 vs. I2-L2), one-tailed (I1 > I2), α = 0.05, `var.equal = FALSE` (Welch's t-test). Report Cohen's d + two-sided 95% CI (Hedges & Olkin 1985 SE approximation). [Amendment 15 (pre-data): `var.equal = FALSE` added explicitly — consistent with M3 TOST and general analysis approach; Cohen's d CI method specified. Analysis script: diff display bug fixed (conf.int[1] was used as the mean-difference value; now computed as I1_mean − I2_mean directly); Cohen's d and two-sided 95% CI added. Commit tick-4297.]
2. **M3 save intention (equivalence):** TOST on self-report M3 (I1-L2 vs. I2-L2), equivalence bounds ±0.5 SD in raw Likert units (bounds = 0.5 × observed M3 SD computed from pooled L2 data), α = 0.05 per one-sided test. **Method: Welch two one-sided t-tests (TOSTER::tsum_TOST, `var.equal = FALSE`; Lakens, 2017 TOST framework for means).** Equivalence established if max(p_lower, p_upper) < 0.05. If equivalence not established, report M3 Cohen's d and 90% CI. [Amendment 19 (pre-data): analysis script was missing this fallback — when `tost_pmax ≥ 0.05`, script only emitted "NOT ESTABLISHED" without reporting Cohen's d + 90% CI. Fixed: conditional block added after TOST result in §4.3 (H4_SUPPORTED path). Uses pooled SD Cohen's d and Hedges & Olkin (1985) SE with Welch df; 90% CI = d ± t(0.95, df_welch) × SE. Commit tick-4303.] [Fixed tick-4249: prior wording "Lakens, 2017 z-test on proportions adapted for Likert" was methodologically incorrect. M3 is a 7-point Likert scale (continuous); the correct TOST uses the t-distribution based `tsum_TOST`, not a z-test for proportions. Contrast with Study 1 H2-tertiary which used a custom z-test for accuracy *proportions* — that is a different scale type requiring a different TOST implementation. Analysis script (piup-study2-analysis.R H2.3) confirmed: `TOSTER::tsum_TOST(..., var.equal = FALSE)`. No protocol impact — only the method description was wrong; bounds, alpha, and verdict logic are unchanged.]
...
**Pre-specified exploratory (Amendment 24):** Add M1 × I interaction term: `glm(m3_click ~ m1_qac * I + L + E, ...)`. Tests whether the calibration intervention (I1 vs. I2) moderates the Q-AC accuracy → download click relationship. Report interaction OR + 95% CI. Not confirmatory.  
**Exploratory (not pre-registered):** Add M1 × L interaction term: `glm(m3_click ~ m1_qac * L + E + I, ...)`. Tests whether label moderates the comprehension → download link. Report interaction OR + 95% CI. Not pre-registered; report as post-hoc exploratory.
...
- **κ ≥ 0.70 gate:** All Q-OE analyses (KW, Dunn's, descriptives) are suppressed if κ < 0.70; raters must adjudicate and rescore before re-running. [Amendment 20 (pre-data): analysis script was missing this gate — KW and M6 descriptives ran unconditionally regardless of κ. Fixed: `kappa_ok` flag added; M6 analysis wrapped in `if (kappa_ok)`. No hypothesis, endpoint, alpha, or verdict change. Commit tick-4304.]
- Kruskal-Wallis test across 8 conditions on `qoe_final`.
- If significant (α = 0.05): Dunn's pairwise post-hoc (Holm correction).
- 15 randomly sampled Q-OE responses per condition published as illustrative examples (random sample drawn before hypothesis testing). [Amendment 18 (pre-data): raw open-text Q-OE column name assumed to be `qoe_open_text`; confirm column name on data collection. Analysis script implements the draw with `set.seed(20260901)` (fixed seed, pre-registered) in a new §3.5 block before section 4 (before any hypothesis outcome is computed), guarded by a column-existence check. Seed value: 20260901. No hypothesis, endpoint, alpha, bound, or verdict change. Commit tick-4301.]

### 6.8 Prior-study sensitivity check (pre-specified)

Re-run H2.1 and H2.4 excluding `prior_receipt_study = 1` participants. Report both primary and sensitivity-check results. [Amendment 17 (pre-data): analysis script was incorrectly excluding `prior_receipt_study = 1` participants from the primary analytic sample (a hard exclusion via `df <- df[...]`), contradicting §5 which specifies this as "not an exclusion — flag only." Fixed: exclusion line removed; replaced with a flag-and-count block matching the browser-fallback pattern. A §6.8 sensitivity block added to the script (after the §9.3 browser-fallback sensitivity checks): re-runs H2.1 (chi-squared, one-tailed) and H2.4 (logistic regression) on `df_no_prior` (prior-study participants removed); reports both primary and sensitivity-check ORs and verdicts; skips (prints not-applicable) if `n_prior_study == 0`. No hypothesis, endpoint, alpha, bound, or verdict criterion change. Commit tick-4301.]
...
**Deception:** None. Cover story (instrument §4): "You are helping researchers evaluate a prototype voting interface. You will interact with a screen that appears immediately after you submit a vote. The interface is a prototype only — there is no real election and no real votes are being collected." [Amendment 22 (pre-data): §10 previously quoted a stale cover scenario ("You have just voted in a simulated election. Take a moment to review your receipt.") that does not appear in instrument §4. Corrected to match the actual instrument wording.]

**Coercion scenario (calibration questions, I2 conditions):** CAL1 (instrument §5) presents a screenshot-request scenario: "If someone asked you to send them a screenshot of your voting receipt to prove how you voted, could they learn your vote choice from the screenshot?" This is a hypothetical conditional; no real coercive pressure is involved. [Amendment 22 (pre-data): §10 previously said "employer verification scenario (same as Study 1 Q3)" — this is inaccurate on two counts: (1) Study 2 CAL1 uses "someone" (generic), not "coercive employer" as in Study 1 Q3; (2) the wording differs materially from Study 1 Q3. Corrected to reflect actual CAL1 question text from instrument §5.]
...
Reduction if H2.3 dropped (N = 160): full study ~$400; total ~$798. (Pilot $200 + full $400 + platform fee 33% × $600 = $198; $200 + $400 + $198 = $798.)
...
| 2026-06-30 | 13 | Status header §4/§9/§10 contingency list corrected; reduced-N budget math fixed; H2.3 body propagation | Status header listed "§4, §9, and §10" as sections to update when H2.3 is dropped. §9 (Open science commitments) and §10 (Ethical considerations) have no N references and require no update; the correct sections are §4 (sampling plan), §7 (pilot N: 80→40), §12 (budget), and §13 (decision framework, remove H2.3 row). Status header updated accordingly. Also fixed: §12 reduced-N budget note stated "total ~$730" which was a math error. Correct calculation: pilot $200 + full $400 + platform fee 33%×($200+$400) = $200+$400+$198 = $798. Additional fix (tick-4270 propagation): the H2.3 body text in §3 also said "Update §4, §9, and §10 accordingly" — same stale list, not fixed by the status-header correction in tick-4264. Corrected to "Update §4, §7, §12, and §13 accordingly" with an explanatory parenthetical matching the status header. Amendment log rows 12+13 re-ordered numerically (13 appeared before 12 — cosmetic insertion-order error). No hypothesis, endpoint, or analysis logic change. | OpenClaw Agent |
| 2026-06-30 | 14 | §4.1 broken §9.3 xref fixed; §6.2 H2.1 browser-fallback sensitivity pre-specified | §4.1 said "see §9.3" — but pre-reg §9 (Open science commitments) has no §9.3 subsection; the correct reference is design note §9.3. Changed to "see design note §9.3". Separately: §4.1 promises a pre-specified browser-fallback sensitivity check for H2.1 (retained in analytic sample, excluded from sensitivity), and the analysis script (line 870) implements it, but §6.2 (H2.1 analysis plan) had no corresponding sensitivity subsection — unlike §6.5 (H2.4) which explicitly details its browser-fallback check. Added "H2.1 browser-fallback sensitivity check (pre-specified)" subsection to §6.2 (parallel to §6.5), specifying: re-run chi-squared excluding browser_fallback=1 participants; report both primary and sensitivity-check ORs and verdicts; note non-applicable if n_fallback=0. No hypothesis, endpoint, alpha, bound, or analysis logic change — documents an already-implemented sensitivity check. Commit tick-4291. | OpenClaw Agent |
| 2026-06-30 | 16 | §6.3 H2.2 null path: E and L marginal means added; dead code removed | §6.3 specifies "report E main effect and L main effect descriptively" in the null-interaction path. Analysis script null path reported 90% CI on the interaction term (via `emmeans::contrast`, `level = 0.90`) but never printed E or L marginal means. Fixed: `emmeans::emmeans(m2_lm, ~ E)` and `emmeans::emmeans(m2_lm, ~ L)` added after the interaction contrast print. Dead code also removed: `int_ci90 <- confint(m2_lm, level = 0.90)` and `int_term <- "..."` were assigned but never printed or used. Null verdict text updated to confirm descriptive reporting. No hypothesis, alpha level, bound, or verdict criterion change. Commit tick-4299. | OpenClaw Agent |
| 2026-06-30 | 15 | §6.4 M4 t-test: `var.equal = FALSE` explicit; Cohen's d 95% CI method specified; analysis script diff display bug fixed | §6.4 item 1 did not specify `var.equal = FALSE` (Welch's t-test) for the M4 t-test, inconsistent with the M3 TOST specification (`var.equal = FALSE`) and the general analysis approach. Added explicitly. Cohen's d 95% CI method now specified: Hedges & Olkin (1985) SE approximation (`sqrt(1/n1 + 1/n2 + d²/(2*(n1+n2-2)))`), two-sided 95% CI. Analysis script fix: the sprintf reporting the M4 mean-difference incorrectly used `t_h23$conf.int[1]` (CI lower bound from the one-tailed test) as the "diff" value; `conf.int[1]` was also printed twice (once as diff, once as CI lower bound). Fixed: `m4_diff` computed directly as `I1_mean − I2_mean`; Cohen's d and two-sided 95% CI added via pooled SD and Hedges–Olkin SE. No hypothesis, endpoint, alpha level, equivalence bound, or verdict criterion change. Commit tick-4297. | OpenClaw Agent |
| 2026-06-30 | 17 | §5 + §6.8: prior_receipt_study script bug fixed — exclusion → flag; §6.8 sensitivity check added to analysis script | Analysis script was incorrectly excluding `prior_receipt_study = 1` participants from the primary analytic sample (`df <- df[is.na(df[[COL_PRIOR_STUDY]]) | df[[COL_PRIOR_STUDY]] != 1, ]`), contradicting §5 which specifies "Prior-study sensitivity flag (not an exclusion)" and step 5 which says "Flag prior_receipt_study = 1 participants". Fixed: exclusion block removed; replaced with a flag-and-count block (matching browser-fallback pattern): computes `n_prior_study` and prints a NOT-excluded note. A §6.8 sensitivity block added to the script (after the §9.3 browser-fallback sensitivity checks, before the summary table): when `n_prior_study > 0`, constructs `df_no_prior` and re-runs H2.1 (chi-squared, one-tailed) and H2.4 (logistic regression) on the prior-study-excluded sample; reports both primary and sensitivity-check ORs and verdicts; when `n_prior_study == 0`, prints not-applicable message. No hypothesis, endpoint, alpha, equivalence bound, or verdict criterion change. Commit tick-4301. | OpenClaw Agent |
| 2026-06-30 | 18 | §6.7 Q-OE random sample: analysis script implementation added | §6.7 specifies "15 randomly sampled Q-OE responses per condition published as illustrative examples (random sample drawn before hypothesis testing)" but the analysis script had no code for this. Fixed: new §3.5 block added before section 4 (before any hypothesis outcome is computed). Block uses `set.seed(20260901)` (fixed, pre-registered seed) and draws up to 15 rows per condition from the raw open-text Q-OE column (`COL_QOE_TEXT = "qoe_open_text"`) with a column-existence guard. Raw text column name (`qoe_open_text`) assumed from instrument §16; confirm exact column name on data collection and update `COL_QOE_TEXT` accordingly. No hypothesis, endpoint, alpha, bound, or verdict change. Commit tick-4301. | OpenClaw Agent |
| 2026-06-30 | 19 | §6.4 H2.3 M3 equivalence null path: Cohen's d + 90% CI fallback added to analysis script | §6.4 specifies "If equivalence not established, report M3 Cohen's d and 90% CI." The analysis script had no code for this fallback — when TOST p_max ≥ 0.05 (equivalence not established), the script only emitted the verdict text "NOT ESTABLISHED" without computing or printing Cohen's d and 90% CI. Fixed: a conditional block added immediately after the TOST result section in the H2.3 analysis (§4.3). When `is.na(tost_pmax) \|\| tost_pmax >= 0.05`, the block computes M3 Cohen's d (pooled SD method) and its 90% two-sided CI using the Hedges & Olkin (1985) SE approximation with Welch df (from `t.test(df_L2_I1$m3_save, df_L2_I2$m3_save, var.equal = FALSE)`) and prints I1/I2 means and SDs. 90% CI: d ± t(0.95, df_welch) × SE. No hypothesis, endpoint, alpha, equivalence bound, or verdict criterion change. Commit tick-4303. | OpenClaw Agent |
| 2026-06-30 | 21 | §8/§9 cross-check: stale pre-registration cross-references corrected in analysis script | Three comments/output strings in `piup-study2-analysis.R` cited "pre-registration §9.x" when pre-registration §9 (Open science commitments) has no subsections — those section numbers (§9.1, §9.3) belong to the design note. Fixed: (1) line 335 comment "Per pre-registration §9.3" → "Per design note §9.3 (pre-registration §4.1)" + Amendment 21 note; (2) line 646 comment "per pre-registration §9.1" → "per design note §9.1 / pre-registration §6.3" + Amendment 21 note; (3) pilot mode output string "per pre-registration §9" → "per pre-registration §7.1" (pilot HT suppression is in §7.1, not §9) + Amendment 21 note; (4) script footer: added missing `piup-study2-drycheck.R` to OSF upload reminder (§9 specifies both scripts); updated "before Study 2 data collection" to "before pilot launch" to match §9 wording. Dry-check: CLEAN (37/40 N; packages OK). No hypothesis, endpoint, alpha, equivalence bound, or verdict criterion change. Commit tick-4305. | OpenClaw Agent |
| 2026-06-30 | 22 | §10 cover story quote + CAL1 description corrected | §10 quoted a stale cover scenario ("You have just voted in a simulated election. Take a moment to review your receipt.") that does not appear in the survey instrument. Instrument §4 (Welcome + Cover Story) says: "You are helping researchers evaluate a prototype voting interface. You will interact with a screen that appears immediately after you submit a vote. The interface is a prototype only — there is no real election and no real votes are being collected." Corrected to match the actual instrument wording. Separately: §10 described CAL1 as "employer verification scenario (same as Study 1 Q3)" — inaccurate on two counts: (1) Study 2 CAL1 uses "someone" (generic), not "coercive employer" as in Study 1 Q3; (2) the wording differs materially from Study 1 Q3. Corrected to reflect actual CAL1 question text from instrument §5: "If someone asked you to send them a screenshot of your voting receipt to prove how you voted, could they learn your vote choice from the screenshot?" No hypothesis, endpoint, alpha, equivalence bound, or verdict criterion change. | OpenClaw Agent |
| 2026-07-01 | 24 | §6.5 H2.4: Q-AC × I exploratory interaction added to analysis script (Amendment 24) | Analysis script: `piup-study2-analysis.R` H2.4 block now includes a second exploratory model (`glm(m3_click ~ m1_qac * I + L + E, ...)`) testing whether the calibration intervention (I1 vs. I2) moderates the Q-AC accuracy → download click relationship. The CHI paper draft (tick-4344, §5 H2.4) described this as "pre-specified exploratory" but the analysis script only contained a Q-AC × L exploratory model — not the Q-AC × I model mentioned in the paper. Amendment 24 formalises the Q-AC × I exploratory as a pre-data pre-specified exploratory analysis: reports interaction OR + 95% CI, labeled NOT CONFIRMATORY. The interpretive note in the script explains the two plausible directions (OR > 1 → calibration amplifies comprehension→download link; OR < 1 → calibration substitutes for it). No confirmatory hypothesis, endpoint, alpha level, equivalence bound, or verdict criterion change. The Q-AC × L exploratory model is retained but its CI is now also reported (minor gap in prior output). Commit tick-4373. | OpenClaw Agent |
| 2026-06-30 | 23 | §6.3 M2 α < 0.70 warning added to analysis script; instrument §2 survey flow M4 position corrected; codebook gap filled | Analysis script: M2 Cronbach's α < 0.70 gate added before H2.2 ANOVA. Prior code printed *** BELOW THRESHOLD *** in the descriptives section but then ran H2.2 unconditionally on the composite; §5.3 says "if α < 0.70, report items individually." Fixed: when `alpha_raw < 0.70`, a warning block fires before H2.2 that prints individual item means and labels the ANOVA result as EXPLORATORY. No hypothesis, alpha, or verdict change. Instrument fixes (4 items): (1) §2 survey flow: M4 was listed at position 11 (after M3 Save Intention) instead of immediately after Q-AC (before Trust Scale) as specified in §11 and pre-registration §5.3 — corrected; (2) §18 codebook: `qoe_open_text` column added (raw Q-OE text, used by analysis script Amendment 18); (3) §14 DM2 note clarified: DM2 is used as a hard exclusion in the analysis script for SC2 slipthrough, consistent with pre-registration §4.1/§6.1 step 3; (4) §20 pilot checklist: split into structural pipeline check (5/cell = N=40) and full pre-registered pilot (10/cell = N=80). Commit tick-4321. | OpenClaw Agent |
| 2026-06-30 | 20 | §6.7 M6 κ gate missing — KW analysis ran unconditionally even when κ < 0.70 | §6.7 specifies "κ ≥ 0.70 required before including Q-OE in any analysis" but the analysis script set no gate: the `kappa_ok` flag was never initialised, so the Kruskal-Wallis + Dunn's post-hoc M6 analysis and the `[EXPLORATORY] M6 Q-OE mean rater score by E` descriptive both ran regardless of the §3 IRR result. Fixed: `kappa_ok <- FALSE` initialised before the IRR block; `kappa_ok <- TRUE` set inside the `κ ≥ 0.70` branch; M6 exploratory analyses (mean by E + KW/Dunn) wrapped in `if (kappa_ok) { ... } else { skip message }`; M4 residual block (previously inside the same `if (!PILOT)` as KW) moved to its own separate `if (!PILOT)` block so it runs unconditionally with respect to κ. Dry-check: CLEAN (37/40 N; packages OK). No hypothesis, endpoint, alpha, equivalence bound, or verdict criterion change. Commit tick-4304. | OpenClaw Agent |
