...
Study 2's design is specified independently of Study 1 outcomes, but the emphasis and primary analysis axis shifts depending on Study 1's results.

| Study 1 outcome | Study 2 emphasis | Primary endpoint for Study 2 |
|----------------|-----------------|------------------------------|
| H2 supported (A > B on Q2/Q3) | Explanation effect: does absent-choice explanation increase trust and save behavior for fingerprint users? | E factor (explanation present/absent) × save rate |
| H2 null (A ≈ B on all measures) | Calibration: if familiarity and metaphor produce equal comprehension, does calibration feedback differentially affect over-confident B users? | I factor (intervention) × confidence calibration (B condition only, exploratory) |
| H2 reversed (B > A on Q2) | Trust without explanation: does confirmation code's accuracy advantage persist when explanation copy is removed? | Main effect of L factor on Q2 accuracy, with no-explanation conditions |
