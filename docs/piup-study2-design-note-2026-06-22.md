...
| H4 supported (B highest confidence, moderate accuracy) | Calibration intervention: does accuracy feedback reduce confidence miscalibration without reducing save rate? | H2.1: E main effect on Q-AC (primary); H2.3 conditional secondary (key reporting focus in H4-supported scenario): I × confidence-accuracy residual (M4, L2 conditions only; pre-specified conditional on H4) |
...
[Fixed tick-4073: 'this receipt' → 'that receipt' - the receipt is hidden before Q-AC is asked (per the transition screen in §6 of the instrument: 'The receipt screen is now hidden. Please answer the following questions from memory.'). 'That receipt' correctly signals retrospective reference. Instrument §8 uses 'that receipt' throughout; design note now matches.]
...
7-point post-receipt Q-AC confidence rating for **all conditions** (N=240): "How confident are you in your answer above?" Placed immediately after Q-AC (M1/§8). [Fixed tick-4246: JONY-ACTION FF resolved (a). Prior draft said 'I2 condition only' with a retrospective CAL-probe question; instrument §11 updated to all-conditions, post-receipt Q-AC confidence. Rationale: H2.3's pre-specified t-test compares I1-L2 vs. I2-L2 on M4 residual, which requires M4 for both I1 and I2 participants. I2-only M4 would make H2.3 untestable as written. Option (a) resolves this: all participants rate Q-AC confidence immediately after the primary comprehension question, before the trust scale.]

**Placement:** M4 appears immediately after Q-AC (M1), on the same comprehension screen, before the Trust block (M2). Asking post-receipt Q-AC confidence right after the accuracy question anchors the rating to the specific judgment just made, minimising retrospective reconstruction error. The earlier retrospective placement (after Trust) was specific to the I2-only construct and is no longer applicable.

Calibration residual = (confidence rating - 1) / 6 - Q-AC accuracy score. [Fixed tick-4073: prior formula said 'confidence rating - actual accuracy score' without the rescaling step. Confidence (raw 1-7) must be rescaled to a 0-1 metric via (x - 1) / 6 before subtracting Q-AC binary accuracy (0 or 1); otherwise the difference is uninterpretable (1-7 scale minus 0/1 scale). The analysis script uses this rescaled formula. Instrument §11 specifies it explicitly; design note now matches.] Positive residual = over-confidence; negative = under-confidence. H4 in Study 1 predicts B (confirmation code) will have positive residual; Study 2 tests whether I2 intervention reduces this residual in the L2 conditions.

**M5 - Verification instruction engagement**

Binary: did the participant expand the "how to verify" section? This is an interaction log captured via the `piup-verify-expanded` postMessage event from the prototype - **there is no corresponding survey question for M5**. The Qualtrics Stimulus block JavaScript listens for this event and writes the result to the `verify_expanded` Embedded Data field. [Fixed tick-4073: 'accordian' typo corrected to 'accordion'; added explicit 'no survey question' note, consistent with instrument §2 Survey Flow note.]
...
Scored 0-2 by two independent raters: [Fixed tick-4075: rubric expanded to match instrument §16 exactly. Prior version had a compressed summary that omitted the two-part score-2 criterion, the score-1 feature-recognition clause, and the score-0 extended examples. Also added tie-breaking rule and `qoe_final` formula, which were absent.]
...
**Analysis column:** `qoe_final = round((r1 + r2) / 2)` after IRR check (consistent with instrument §16 and analysis script).

κ ≥ 0.70 required before including Q-OE in any pre-specified analysis. If κ < 0.70 on pilot data, raters adjudicate disagreements and rescore using the revised rubric. Log rubric revision as a protocol amendment.
...
If F interaction significant (α = 0.05): simple effects of E within L1 and L2 separately (Welch’s t-test, `var.equal = FALSE`; pooled ANOVA MSE not used for simple effects).
...
Mitigation (two layers): (1) **Prolific "previous studies" filter** - exclude participants who completed Study 1 Prolific study ID. (2) **DM4 (within-survey self-report cross-check)** - participants are asked at the demographics stage: "Have you participated in a previous study about voting receipts, voting confirmations, or post-vote screens in the past 6 months?" [Fixed tick-4073: original wording 'voting receipts or confirmation codes' updated to match instrument §14 DM4 wording exactly: 'voting receipts, voting confirmations, or post-vote screens'; also added DM4 as the second mitigation layer - the design note previously only mentioned the Prolific screener.] DM4 respondents who answer "Yes" are flagged as `prior_receipt_study = 1` in the dataset; they are retained in the analytic sample but excluded from the primary analysis and included in a pre-specified sensitivity check. This relies on self-report; it is imperfect but provides a within-survey cross-check against Prolific filter failures.
...
| 2026-06-28 | New file - Study 2 Qualtrics setup guide | Created `docs/qualtrics-setup-guide-study2-2026-06-28.md` (tick-4064, commit a274bc1). Covers: (a) §A Prototype host setup - minimal React app (study2-host) with URL param schema `?condition=L1E1I1`, VoteReceipt props mapping (L→labelVariant, E→explanationVariant; I factor is Qualtrics-only), and postMessage protocol (`piup-ready`/`piup-download-click`/`piup-verify-expanded`). (b) §2 Survey Flow - 15 Embedded Data fields, Qualtrics Randomizer (8 sub-elements, even distribution, 8 quotas n=30/cell). (c) §5 Stimulus block - iframe embed, 90s countdown JS, 8s browser-fallback timeout, postMessage listener. (d) §6 Calibration block (I2 only). (e) §8 Comprehension block - Q-AC (M1), calibration_confidence (M4), trust matrix 4-item (M2), save_intention (M3), Q-OE (M6). (f) §14 Variable name checklist (25 columns). (g) §15 Pre-launch checklist + §16 Pilot instructions. Note: guide §A.3 sample code used a non-existent `label` prop - corrected to `labelVariant` (tick-4065). | OpenClaw Agent |
| 2026-06-28 | Bug fix - §A.3 prop error in Study 2 Qualtrics setup guide | Fixed `label` → `labelVariant` in `qualtrics-setup-guide-study2-2026-06-28.md`. (a) `§A.1` mapping table: `label prop` → `labelVariant prop`; values `"vote fingerprint"`/`"confirmation code"` → `"fingerprint"`/`"confirmation-code"` (correct ReceiptLabelVariant keys). (b) `§A.3` sample code: variable renamed `const labelVariant = condition.startsWith('L1') ? 'fingerprint' : 'confirmation-code'`; JSX prop changed from `label={label}` to `labelVariant={labelVariant}`. Root cause: `VoteReceipt` has no `label` prop - the correct prop is `labelVariant?: ReceiptLabelVariant` (type: `'fingerprint' \| 'confirmation-code' \| 'nullifier' \| 'receipt-id'`). Using a non-existent prop would silently pass `undefined`, falling back to the default `labelVariant='fingerprint'` regardless of condition; L2 participants would always see 'confirmation code' display text but the prop would have no effect. The `condition_label` Embedded Data field (for Qualtrics piped text) is unaffected - that is set in the Qualtrics Randomizer sub-elements, not in the React component. (tick-4065) | OpenClaw Agent |
| 2026-06-28 | New file - Study 2 survey instrument | Created `docs/piup-study2-survey-instrument-2026-06-28.md` (tick-4069, commit 852e165). The Study 2 equivalent of the Study 1 instrument (`piup-study1-survey-instrument-2026-06-22.md`). Covers: (a) All 8 conditions specified (L × E × I; code, label, explanation, calibration columns). (b) Screener SC1/SC2 (matching Study 1 Amendment 5 CS/SE student exclusion). (c) Calibration block (I2 only) with exact CAL1/CAL2/CAL-FEEDBACK wording. (d) Stimulus block: iframe embed, 90s countdown, 8s browser-fallback, postMessage logging JS. (e) Primary measure Q-AC (M1, binary). (f) McKnight 4-item trust scale (M2: TI1, TI2, TC1, TC2, 1-7 Likert). (g) Save intention M3 (7-point scale). (h) Calibration confidence M4 (I2 only). (i) Q-OE absent-choice explanation M6 with 0-2 rubric (κ ≥ 0.70 required). (j) Two attention checks AC1/AC2. (k) Demographics DM1-DM4 (including DM4 prior study filter for Study 1 participants). (l) Variable codebook (25 fields matching COL_* constants in piup-study2-analysis.R exactly). (m) Pre-launch checklist (14 items). | OpenClaw Agent |
| 2026-06-28 | §7.2 M6 rubric expanded to match instrument §16 (tick-4075) | Design note M6 rubric was a compressed summary that diverged from instrument §16 in four ways: (1) Score-2 lacked the explicit two-part criterion (privacy/secrecy AND intentional design or concrete consequence); (2) Score-1 lacked 'Responses that correctly recognise absence as a feature (not an error) but cannot articulate why = 1'; (3) Score-0 lacked the 'it was encrypted' example and the 'vote IS shown somewhere' scenario; (4) Missing tie-breaking rule (raters discuss → consensus; if no consensus, lower score) and `qoe_final = round((r1 + r2) / 2)` formula. Fixed to match instrument §16 exactly. κ threshold text updated: now specifies pilot-data adjudication and rubric-revision amendment, consistent with instrument §16 IRR threshold wording. Pre-pilot; no protocol change, no hypothesis change, no question wording change. | OpenClaw Agent |
| 2026-06-29 | §7.2 M4 scope fix: I2-only retrospective → all-conditions post-receipt Q-AC (tick-4246) | JONY-ACTION FF resolved (a). H2.3's pre-specified t-test compares I1-L2 vs. I2-L2 on M4 residual; this requires M4 for both I1 and I2. The prior I2-only M4 (retrospective CAL-probe confidence) made H2.3 untestable as written. Fixed: instrument §11 updated to all-conditions, post-receipt Q-AC confidence ('How confident are you in your answer above?'), placed immediately after Q-AC (before Trust block). Guide FF flag removed. Analysis script updated: (1) COL_CALIB_CONF comment; (2) synthetic data generator (calib_conf now simulated for all N=240); (3) H2.3 section: JONY-ACTION HH block removed, df_L2 filter fixed (no longer drops I1 rows). Design note §7.2 M4 updated to match. No OSF amendment needed (Study 2 not yet pre-registered). | OpenClaw Agent |
| 2026-06-28 | §3 H2.3 label fix: co-primary → conditional secondary (tick-4074) | §3 contingency table row 4 (H4 supported) said 'H2.3 co-primary' - inconsistent with §9.1 ('pre-specified conditional test') and §10.3 ('conditional secondary test'). Tick-4027 reviewed row 4 only for H2.1's primary label correctness; H2.3's own designation was not rechecked against §9.1/§10. Fixed to 'H2.3 conditional secondary (key reporting focus in H4-supported scenario)'. H2.3 receives elevated narrative prominence in the H4-supported scenario but does not replace or equal H2.1's pre-registered primary status - consistent with §9.1 and §10.3. Also: confirmed H2.1 primary/secondary labels across all 5 §3 rows are consistent with §9.1 fixed-primary language; confirmed §9.3 browser-fallback retention rule and H2.1+H2.4 sensitivity checks are consistent with analysis script implementation (lines 843-903). No other inconsistencies found. | OpenClaw Agent |
| 2026-06-28 | Design note ↔ instrument consistency fixes (tick-4073) | Four inconsistencies between design note and instrument identified and fixed. (1) §7.1 Q-AC stem: 'Looking at **this** receipt' → 'Looking at **that** receipt' - receipt is hidden per transition screen when Q-AC is asked; instrument §8 uses 'that receipt' throughout. (2) §7.2 M4 residual formula: added `(x - 1) / 6` confidence rescaling step (0-1 metric before subtracting Q-AC binary accuracy); plain 'confidence rating - accuracy score' was dimensionally mismatched (1-7 minus 0/1). Instrument §11 added this in tick-4070; design note now matches. (3) §7.2 M4: added explicit placement rationale - M4 appears after Q-AC and trust blocks (intentional; post-receipt experience informs retrospective confidence judgment). (4) §7.2 M5: added 'no survey question' note (M5 is a behavioral log via postMessage); fixed 'accordian' typo → 'accordion'. (5) §11.4: updated DM4 wording to match instrument §14 exactly ('voting receipts, voting confirmations, or post-vote screens'); added DM4 as the second mitigation layer alongside the Prolific screener. | OpenClaw Agent |
...
- Das, S., Kramer, A.D.I., Dabbish, L.A., and Hong, J.I. (2015). "The Role of Social Influence in Security Feature Adoption." _CSCW 2015:_ 1416-1426. [Fixed tick-4051: (1) year 2014→2015; (2) second author Kim, T.H.-J.→Kramer, A.D.I. - confirmed via DBLP (dblp.org/rec/conf/cscw/DasKDH15.html) and ACM DL (doi:10.1145/2675133.2675225). This is a distinct paper from Das, Kim et al. SOUPS 2014 (line 480); different co-author set (Kramer vs Kim) and different venue confirm they are independent publications.]
