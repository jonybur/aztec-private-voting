...
The four studies follow directly from the PIUP's core claims. Study 1 — pre-registration complete and ready for OSF upload, with pilot launch planned once the filing is confirmed — is a four-condition between-subjects online survey (Prolific, N=280) isolating the effect of identifier label ("fingerprint" vs. "confirmation code" vs. "nullifier" vs. "receipt ID") on comprehension of receipt semantics: does label choice affect whether users correctly believe the identifier cannot reveal their vote? The confirmatory plan pre-registers 14 tests across four hypothesis families (H1–H4), with the primary endpoint on a two-item mental model probe and Holm-corrected family-level α. Study 2, whose design is contingent on Study 1's H4 outcome, is a 2×2×2 factorial (identifier label × absent-choice explanation × calibration intervention) — interactive prototype, Prolific, N=240 (8 cells × 30). It tests what Study 1 cannot: whether users correctly interpret absent vote content as a design choice rather than an error, whether comprehension converts to behavioral intention (self-reported save likelihood, observed download interaction), and whether an accuracy-feedback calibration intervention at receipt display time closes the overconfidence gap the H4 prediction identifies. Study 3 is a field experiment in an actual DAO deployment: a between-subjects test of whether displaying an aggregate verification count to voters at the moment of ballot receipt increases the rate at which they return to verify after the vote closes — a direct application of the social proof mechanism from Das et al. (CCS 2014) to deferred security verification in a private system (pre-registered feasibility pilot, N ≥ 80; powered replication N ≥ 280 at OR = 2.0, 80% power). The Aztec Private Voting contract's public `verify_vote_counted()` view function enables privacy-preserving vote-inclusion verification (returning whether a given ballot fingerprint was counted, without revealing vote content); a host-server backend aggregates these anonymous verification calls to generate the social proof counter — enabling the manipulation while preserving the privacy property under study. Study 4 is a 2×2 between-subjects vignette experiment (N=160) crossing receipt UI design — countdown-only (copy instruction: "sharing is safe after vote close") versus UI-lock that disables the share button until vote close — against coercion pressure level (moderate social request / high job-threat demand). It tests whether the UI-lock produces genuine social deniability: a voter facing coercion can truthfully claim the application will not allow sharing, converting a normative instruction ("you shouldn't") into a structural fact ("you can't"). Two pre-registered DVs operationalise this: DV1 (self-reported sharing intent, 7-point scale, primary confirmatory) and DV2 (perceived social deniability — how convincing is "the app won't let me" as an excuse under pressure, secondary confirmatory). The interaction hypothesis predicts the lock's sharing-intent reduction is larger under high pressure, where a technically-grounded refusal is harder to override than a normative one; H4.3 separately tests whether UI-lock participants rate the technical excuse as more socially convincing. Study 4 tests PIUP Invariant 2 at the social layer — the claim that UI enforcement, not copy instruction alone, is required for coercion resistance — and is pre-registered before IRB submission.
...
- The four-study agenda is the academic substance; it must stay intact
...
I have a working implementation compiled for Aztec Network v5, a pre-registration for Study 1 complete and ready for OSF upload, and a full four-study agenda in my research statement. Happy to share that.
