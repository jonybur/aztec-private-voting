...
R (v ≥ 4.3). Planned packages: `stats`, `PropCIs` (Wilson CIs), `multcomp` (Holm corrections), `irr` (Cohen's κ). [Amendment 4: DescTools removed — CramerV/OddsRatio replaced with base-R.] [Amendment 10: TOSTER removed — TOSTER::tsum\_TOST is for means (t-distribution), not proportions; H2-tertiary TOST implemented as custom z-test (tost\_prop()) per Lakens (2017); TOSTER was never called in the script.] Analysis script uploaded to OSF alongside this pre-registration.
