...
**Invariant 2 behavioural validation.** Studies 1 and 2 test comprehension (does the voter understand what the receipt withholds?) and verification behaviour (does the voter return to verify?). Neither tests whether PIUP's Invariant 2 UX enforcement — the temporal UI-lock that disables download before vote close — produces the social deniability it is designed to afford. The social deniability claim is: a voter facing coercion can truthfully say "the app won't let me share this" (a technical fact, not a personal refusal), and this excuse is more effective than a normative one under adversarial pressure. This claim is unverified. A pre-registered 2×2 between-subjects vignette experiment (N = 160; UI-lock present/absent × coercion pressure moderate/high; primary outcome: sharing intent on a 7-point scale; powered for f = 0.25, 86% power) is the appropriate confirmatory test. The interaction hypothesis — that the UI-lock's sharing-intent reduction is larger under high-pressure scenarios — follows directly from the deniability mechanism: technical facts cannot be overridden by adversarial authority; normative resistance can. This study is pre-registered. The design (`docs/piup-study4-temporal-coercion-vignette-2026-07-01.md`), OSF pre-registration text (`docs/piup-study4-osf-prereg-2026-07-01.md`), Qualtrics implementation guide, dry-check validation script (N=200 synthetic; all 10 sections pass), and Prolific setup guide (`docs/prolific-setup-guide-study4-2026-07-01.md`) are complete. The study is ready to launch once the OSF pre-registration is filed.
