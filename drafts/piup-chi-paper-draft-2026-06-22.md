...
_Status: All sections written. §4.6 Results pending Study 1 data collection (2026-Q3 pilot). Submission-clean pending OSF amendments O+T. Four-study arc: §7 updated tick-4397; abstract + §1.3 updated tick-4402. Study 4 DV1/DV2 dual-operationalisation propagated to abstract + §1.3 + §7 at tick-4418. §7 conclusion para 3 compressed –140w at tick-4467. Volkamer 2022 §1.4 tick-4490. §7 invariants compliance tick-4491. §6.5 Study 4 hypothesis inventory corrected (H4.1 added) tick-4501._
_Word count: ~11,692 words (tick-4501: §6.5 H4.1 hypothesis added +31w; tick-4497: §3.3 severity descriptor +1w; tick-4496: §2.2 Alt-4 redundancy trim –27w; tick-4495: no net change; tick-4494 net: 11,687w after abstract trim –69w tick-4493, §2.3 trim –42w tick-4492). 18 references. §4.6 fill-in adds +244–404w → final ~11,936–12,096w: ✅ lower bound fits cap (64w buffer); trim (~96w) needed only if §4.6 fills at upper bound. CHI cap 12,000 incl. refs, excl. tables. Abstract: 150 words (at CHI abstract cap). Open actions: JONY-ACTION O (OSF Amendment 5) + JONY-ACTION T (OSF Amendments 12+13+14). Also open: Amendments 19, 19b (pre-pilot). Volkamer 2022 §1.4 applied tick-4490 — Jony to confirm or revert. See docs/jony-action-brief-2026-07-02.md._

---

## Abstract

Submission confirmations carry an implicit claim: the system received what you sent. In private cryptographic voting, this creates a coercion surface — a receipt showing vote choice can be demanded as compliance proof. We present **PIUP** (Proof-of-Inclusion Pattern): a design class that confirms participation without revealing content using *protective absence* — named omission of the submitted choice. We instantiate PIUP in Aztec Private Voting and report four pre-registered studies: Study 1 (N=280) tests label effects on privacy mental models; Study 2 (N=240) tests explanation and calibration effects; Study 3 (N≥80) tests social-proof effects on post-vote verification return rates; Study 4 (N=160) tests whether a UI temporal lock produces coercion deniability (primary DV: sharing intent; secondary: perceived deniability of the technical excuse). PIUP formalises three invariants (surrogate independence, surrogate privacy in transit, minimal receipt content) and one named limitation: vote choice appears in current Aztec calldata, addressable at the application layer.
...
The PIUP formalises the design response. The Status line anchors the receipt: the user sees confirmation that their ballot was recorded. Invariant 1 ensures the submission token is not derivable from the vote choice, the voter's identity, or any observable system state, and is verifiable against a public ledger. Invariant 2 requires the token be kept private until the vote closes. Invariant 3 ensures no choice-revealing field appears in the receipt. Protective framing addresses the hardest part: it explicitly names the absent content and reason for its absence before the user's default failure-inference can form. The Aztec Private Voting instantiation (§3) demonstrates all three invariants in a working ZK testnet deployment. Invariant 1 is satisfied at the token-generation layer — `receipt_id` is derived from a random seed independent of vote choice and voter identity — with one known pre-M3 gap: `receipt_id` and `vote_choice` appear together in `record_vote` calldata, enabling external linkability through on-chain archival rather than through the token's intrinsic properties (§3.3, §6.5). Invariant 2 is enforced via a temporal UI-lock on the download affordance: `VoteReceipt.tsx` disables the download button before vote finalisation, so a voter facing coercion cannot share the token even under pressure; Study 4 tests whether this technical constraint produces the social deniability Invariant 2 is designed to afford. Invariant 3 is enforced at the receipt artifact layer — VoteReceipt.tsx renders the full four-component PIUP receipt structure and the downloaded JSON file omits vote_choice.

The empirical argument runs on two threads. First, label and protective framing must be simultaneously correct. Protective framing cannot fully override the wrong mental model on privacy-specific items when the token label's representational schema imports the eCommerce-evidence association (§6.2). If H2 is supported, "vote fingerprint" holds the privacy-mental-model advantage over "confirmation code" on the privacy-specific items Q2 and Q3 (§4.4, §4.5) while the overall accuracy composite remains equivalent — a dissociation the user cannot self-detect. If H4 is also supported, "confirmation code" simultaneously reduces onboarding friction and degrades the privacy mental model; the deficit is not apparent to the user. The label and framing must be correct simultaneously; neither is sufficient alone. Study 2 (§5) tests whether explanation alone is the load-bearing element — whether a familiar label can be rehabilitated through protective framing without substitution. The second thread is generalisation: Studies 3 and 4 test the social-proof and coercion-deniability mechanisms that the receipt design embeds (§6.4–6.5), with participant populations and study contexts closer to real governance settings.
...
**📏 WORD COUNT - UPDATED tick-4504 (2026-07-02):** Body text (annotations + submission notes stripped, refs included): **~11,692 words** (tick-4501: §6.5 H4.1 hypothesis added +31w; tick-4497: §3.3 severity +1w; tick-4496: §2.2 Alt-4 trim –27w; tick-4494 net: 11,687w after abstract trim –69w tick-4493, §2.3 trim –42w tick-4492, §7 invariants fill +107w tick-4491, Volkamer 2022 +22w tick-4490). 18 references. UNDER the 12,000 desk-rejection threshold. §4.6 contains ~145 [SLOT] markers; when filled, word count rises +244–404 → final: ~11,936–12,096 words. **Lower bound 11,936w fits cap (64w buffer); trim (~96w) only needed if §4.6 fills at upper bound; do not pre-cut.** Trim target (if needed): §4.6 narrative or §6.4 examples, NOT §6.1/§6.3/§7. DO NOT cut §6.1 (core design argument), §6.2 (trimmed), §6.3 (feedback inversion), §7 (conclusion), §4.5 (~440 words - done).
...
7. ✅ CHI 2027 word limit and formatting confirmed (tick-4271): 5,000-8,000 encouraged; 12,000 max. Body currently **~11,660 words (under limit)** [tick-4496: §2.2 Alt-4 redundancy –27w; prior 11,687 tick-4494; 11,683 tick-4493; 11,705 tick-4490; 11,683 tick-4467; 11,823 tick-4418; 11,787 tick-4397; 11,535 post-trim tick-4332; 18 refs]. After §4.6 slot fill-in: ~11,905–12,065 — light trim (~65w) only needed if §4.6 fills at upper bound; lower-end fill clears under 12,000. Do not pre-cut: trim only after §4.6 is filled.
