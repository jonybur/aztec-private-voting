...
Zero-knowledge proof systems offer a partial technical resolution. Aztec's ZK rollup makes it possible in principle for a voter to prove eligibility and submit a ballot without revealing the ballot's contents in public calldata. At the private state layer, ZK achieves what it is designed for: the system records a nullifier (a unique commitment derived from the ballot) and the aggregate tally — but no persistent state links the voter to their choice. A named limitation remains at the calldata layer: in the current contract architecture, the vote choice appears as a plaintext argument in the contract's public accounting step, visible at submission time (§1.1). State-level storage of the choice is eliminated; calldata exposure at the moment of submission is not.
...
**H2 (pre-registered confirmatory; m = 3; primary endpoint):** A ≈ B on overall accuracy composite (TOST, ±10 pp); A > B on Q2 (primary endpoint) and Q3 (secondary). Q2(A>B) is the single pre-specified primary endpoint for the study.
...
**Mental model quality (exploratory; free text).** After Q1-Q4, participants answered: "In your own words: what does your [LABEL] prove about your vote?" Scored 0–2 using a two-dimensional additive rubric (full rubric: survey instrument §11). Dimension 1 (Inclusion): scores 1 if the response states the identifier proves the vote was counted, included, or processed by the system; 0 otherwise. Dimension 2 (Non-leakage): scores 1 if the response states the identifier does NOT prove the voter's specific choice — i.e., that the choice is not revealed; 0 otherwise. Total score = Dimension 1 + Dimension 2: 0 = neither concept present; 1 = exactly one concept present (inclusion alone or non-leakage alone); 2 = both concepts present. Two independent raters; κ ≥ 0.70 required before any MQ analysis is included in pre-registered results. If κ < 0.70, raters adjudicate disagreements and rescore before analysis. Mental model quality is excluded from composite accuracy and analysed separately (§4.5).
...
Norman's (1988) central feedback principle — that the system must send back to the user information about what action was done and what result was accomplished — is a design resource for most interface contexts. The principle assumes that the relevant system state is something that happened: a file was saved, a message was sent, a payment was processed. The correct design response is to tell the user what happened. [JONY-ACTION H: The prior draft quoted Norman as 'always keep the user informed about what is going on' in quotation marks — that exact phrasing is Nielsen's Heuristic #1 (1994), not a verbatim Norman (1988) DOET quote. Replaced with paraphrase of Norman's feedback definition above. Before CHI submission: verify against DOET p. 27 (approx) and either restore with page number or confirm paraphrase is sufficient; optionally add Nielsen (1994) alongside Norman if citing the heuristic framing.]
